...
const CountdownTimer = (props: Props) => {
  const { targetDate, className, webinarId, webinarStatus } = props;
  ...
  const formattedNumber = (num: Number) => num.toString().padStart(2, "0");

  const splitDigits = (num: Number) => {
    const formatted = formattedNumber(num);
      ...
      if (difference <= 0) {
        if (!isExpired) {
          ...
          if (webinarStatus === WebinarStatusEnum.SCHEDULED) {
            ...
            // call the async function saperately
            updateStatus();
          }
        }
        return {
